...
class TypesenseAdapter(SearchEngineAdapter):
    ...
    # @property
    # def _ts_connection_class(self):
    #     return typesense.RequestsHttpConnection
    ...
    def index(self, records) -> None:
        ts = self._ts_client
        # Convert records to JSONL string (one JSON object per line)
        jsonl_data = "\n".join([json.dumps(record) for record in records])
        try:
            res = ts.collections[self._index_name].documents.import_(
                jsonl_data, {"action": "upsert"}
            )
        except typesense.exceptions.ObjectNotFound as e:
            _logger.warning(
                f"{self._index_name} not found, skip updating alias and "
                f"creating a new index (collection)!\n\n{e}"
            )
            self.reindex()
            self.index(records)
        # Validate number of successfully indexed documents
        results = [json.loads(line) for line in res.splitlines()]
    ...
    def _get_current_aliased_index_name(self) -> str:
        ...
        try:
            ...
            _logger.warning(
                f"current_aliased_index_name not found, skip updating alias and "
                f"creating a new index (collection)!\n\n{e}"
            )
    ...
    def reindex(self) -> None:
        ...
        if not current_aliased_index_name:
            self.settings()
            current_aliased_index_name = self._get_current_aliased_index_name()
